...
CAPTURE_LIMIT = 1
EXPERIMENT_NAME = "06_closed_loop_after_signfix"

# Must match Core_V5_5_LOOP_APPLY exactly.
# The numeric fields below are the applied RELATIVE deltas, not absolute
# machine coordinates.
CAL_POINTS = [
    ("AFTER_SIGNFIX", 39705, 8844),
]
...
def write_calibration_summary(session_dir, records, total_count):
    ...
    loop_path = os.path.join(session_dir, "loop_after_signfix.txt")

    lines = [
        "Closed-loop AFTER SIGNFIX summary",
        "=================================",
        "total_captures=%d" % total_count,
        "current_pixel=(%.4f,%.4f)" % (1101.8153000000, 1142.3672000000),
        "target_pixel=(%.4f,%.4f)" % (TARGET_U, TARGET_V),
        "before_pixel_error=203.7052",
        "applied_delta_x=39705",
        "applied_delta_y=8844",
    ]

    if not records:
        lines.append("AFTER_NOT_CAPTURED")
    else:
        r = records[0]
        lines += [
            "found=%d" % r["found"],
        ...
        if r["found"]:
            after_error = float(
                np.hypot(TARGET_U - r["u"], TARGET_V - r["v"])
            )
            reduction = 203.7051785054 - after_error
            reduction_pct = (
                100.0 * reduction / 203.7051785054
                if 203.7051785054 > 0.0 else 0.0
            )

            lines += [
                "after_pixel=(%.4f,%.4f)" % (r["u"], r["v"]),
                "after_pixel_error=%.4f" % after_error,
                "error_reduction_px=%.4f" % reduction,
                "error_reduction_percent=%.2f%%" % reduction_pct,
            ]

            if after_error < 203.7051785054:
                lines.append("CLOSED_LOOP_RESULT=IMPROVED")
            else:
                lines.append("CLOSED_LOOP_RESULT=NOT_IMPROVED")
        else:
            lines.append("CLOSED_LOOP_RESULT=TARGET_NOT_FOUND")
    ...
    print("loop result saved:", loop_path)
...
def work_thread(cam=0, pData=0, nDataSize=0):
    ...
    with open(
        ...
        while not g_bExit:
            ...
            print(
                "\n[%02d/%02d] %s applied_delta=(%d,%d) Width[%d], Height[%d], nFrameNum[%d]"
                % (
                    capture_index,
                    CAPTURE_LIMIT,
                    point_name,
