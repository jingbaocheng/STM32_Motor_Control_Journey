 ...
 * 0x2609133A = V5.5 SAFE 3x3 affine-calibration tool.
 */
volatile uint32_t g_build_id = 0x2609133AU;
...
/*
 * SAFE 3x3 capture order.
 *
 * The previous formal run showed all X=79920 points were grossly abnormal,
 * while X=96304 and X=112688 produced a stable local mapping after arrival.
 * Therefore this closeout grid stays inside that proven X region:
 *
 *   X = 96304, 104496, 112688        (8192-count spacing)
 *   Y = 20480, 36864, 53248          (16384-count spacing)
 *
 ...
 *   P0 ---- P1 ---- P2
 *
 * P0 is the first CameraReady capture after the single Home sequence.
 * Every following point is exactly one grid step away from the previous one.
 */
static const int8_t cal_grid_x[CAL_3X3_POINT_COUNT] =
{
     0,  1,  2,  2,  1,  0,  0,  1,  2
};

static const int8_t cal_grid_y[CAL_3X3_POINT_COUNT] =
{
    -1, -1, -1,  0,  0,  0,  1,  1,  1
};

volatile uint8_t g_cal_point_index = 0U;
volatile uint8_t g_cal_capture_count = 0U;
volatile uint8_t g_cal_sequence_done = 0U;
volatile int64_t g_cal_current_x = CAMERA_READY_X_DEFAULT;
volatile int64_t g_cal_current_y = CAMERA_READY_Y_DEFAULT;

/* A physical KEY0 release is required between calibration advances. */
static uint8_t cal_key_release_required = 0U;

/* KEY0 level/hold tracking used by Home start and calibration advance. */
...
/*
 * Use KEY0 as a one-shot "advance to next calibration point" command.
 * The press that started Home is never allowed to carry through into P1.
 */
static uint8_t Cal_KeyAdvanceRequested(void)
{
    if (CAL_3X3_ENABLE == 0U)
    {
        return 0U;
    }
...
static void Cal_LoadPoint(uint8_t point_index)
{
    int64_t x;
    int64_t y;

    x = CAMERA_READY_X_DEFAULT +
        ((int64_t)cal_grid_x[point_index] * CAL_3X3_X_STEP_COUNTS);
    y = CAMERA_READY_Y_DEFAULT +
        ((int64_t)cal_grid_y[point_index] * CAL_3X3_Y_STEP_COUNTS);

    g_camera_ready_x_target = x;
    g_camera_ready_y_target = y;
...
/*
 * Advance from the current captured point to the next grid point.
 * Motion is still owned by the normal System/Motion state machine:
 * absolute move -> encoder verification -> 500 ms settle -> PB0 trigger.
 */
static uint8_t Cal_StartNextPoint(void)
{
    ...
    next_point = (uint8_t)(g_cal_point_index + 1U);

    if (next_point >= CAL_3X3_POINT_COUNT)
    {
        return 0U;
    }

    g_cal_point_index = next_point;
    Cal_LoadPoint(g_cal_point_index);
...
static uint8_t System_StartHome(void)
{
    ...
    /* The Home-start press must be released before P1 can be requested. */
    cal_key_release_required = 1U;
    ...
    MotionController_t *motion_y)
{
    ...
    g_cal_capture_count = 0U;
    g_cal_sequence_done = 0U;
    g_cal_current_x = CAMERA_READY_X_DEFAULT;
    g_cal_current_y = CAMERA_READY_Y_DEFAULT;
    cal_key_release_required = 0U;
    Cal_LoadPoint(0U);
...
void System_Task(void)
{
    ...
    switch (system_state)
    {
        ...
        case SYS_WAIT_HOME_KEY:
            ...
            /*
             * Normal path: one physical KEY0 press held for ~50 ms.
             * Debug escape hatch: set g_debug_home_request=1 in Watch if the
             * laboratory button ever blocks progress.  This is intentionally
             * NOT a second motion owner; it is consumed here by ControlTask.
             */
            if ((g_debug_home_request != 0U) ||
        ...
        case SYS_CAMERA_TRIGGER_WAIT:
                ...
                CAMERA_TRIGGER_PULSE_MS)
            {
                ...
                if ((CAL_3X3_ENABLE != 0U) &&
                    (g_cal_capture_count < CAL_3X3_POINT_COUNT))
                {
                    g_cal_capture_count++;

                    if ((g_cal_point_index ==
                         (CAL_3X3_POINT_COUNT - 1U)) &&
                        (g_cal_capture_count >= CAL_3X3_POINT_COUNT))
                    {
        ...
        case SYS_CAMERA_READY:
            ...
            /*
             * Formal 3x3 calibration:
             *   P0 is captured automatically at CameraReady after Home.
             *   Each subsequent KEY0 press advances exactly one grid point.
             *   There is no second Home between P0 and P8.
             */
            if ((CAL_3X3_ENABLE != 0U) &&
                (g_cal_sequence_done == 0U) &&
            ...
            /*
             * Pure camera repeatability test / debug re-trigger.
             * Do not use this during the formal 3x3 run because it would add
             * an extra frame that has no new calibration coordinate.
             */
