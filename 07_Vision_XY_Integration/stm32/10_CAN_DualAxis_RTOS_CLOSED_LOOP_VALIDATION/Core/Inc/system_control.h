...
typedef enum
{
    ...
    /* One-shot visual correction build: no Home in the normal APPLY path. */
    SYS_LOOP_APPLY_X_START   = 15U,
    SYS_LOOP_APPLY_X_WAIT    = 16U,
    SYS_LOOP_APPLY_Y_START   = 17U,
    SYS_LOOP_APPLY_Y_WAIT    = 18U,
    SYS_LOOP_APPLY_SETTLE    = 19U,
    SYS_LOOP_APPLY_TRIG_START= 20U,
    SYS_LOOP_APPLY_TRIG_WAIT = 21U,
    SYS_LOOP_DONE            = 22U
} SystemState_t;
...
/*
 * CAL_3POINT diagnostics:
 *   0 = P0 (CameraReady)
 *   1 = P1 (X + step)
 *   2 = P2 (Y + step, X returned to P0)
 * g_cal_sequence_done becomes 1 only after the P2 camera trigger completes.
 */
extern volatile uint8_t g_cal_point_index;
extern volatile uint8_t g_cal_sequence_done;

/* LOOP_APPLY diagnostics. */
extern volatile int32_t g_loop_apply_dx;
extern volatile int32_t g_loop_apply_dy;
extern volatile uint8_t g_loop_apply_done;
